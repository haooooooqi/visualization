...
def fetch_video(handle):
    urls = get_everstore_urls([handle])
    multi = get_curl_handles(1)
    vids, success = download(multi, urls, num_retries=2)
    if success[0]:
        return vids[0]
    else:
        return None
...
def get_video_container(handle, multi_thread_decode=False):
    # Local data are located under `/data/local/packages/`.
    if "/data" in handle:
        # Use local data.
        ...
        # Use everstore data.
        vid = fetch_video(handle)
        if vid is None:
            return None
        else:
            raw_video = vid.getvalue()
            return raw_video
